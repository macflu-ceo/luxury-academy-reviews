...
- 제목
- 사진 (선택)
- 금액 세 칸 (선택) — 셋 다 비우면 그 후기에는 금액이 표시되지 않는다.
  세 번째 칸을 비워두면 두 번째에서 첫 번째를 뺀 값이 자동으로 들어간다.
  각 칸의 이름은 드롭다운에서 고른다. 기본값은 공급가 / 정가 / 차액이고,
  선택지는 `/admin` 맨 아래 **금액 이름 목록** 에서 한 줄에 하나씩 추가한다.
- 내용 — 빈 줄 하나로 문단이 나뉜다.
...
상담신청이 들어오면 구글 시트 `후기페이지_상담신청` 탭에도 한 줄씩 붙는다.
원본은 Blob 이고 시트는 보기용 사본이다. **시트가 실패해도 상담신청 접수는 영향받지 않는다.**

- 환경변수 `GOOGLE_SERVICE_ACCOUNT_EMAIL` / `GOOGLE_PRIVATE_KEY` / `GOOGLE_SHEET_ID`
  (창업설명회 랜딩 프로젝트와 같은 값)
- `/admin/consults` 의 **시트와 맞추기** — 저장소에는 있는데 시트에 빠진 신청만 채운다.
  여러 번 눌러도 중복되지 않는다 (신청 시각 + 전화번호로 구분).
